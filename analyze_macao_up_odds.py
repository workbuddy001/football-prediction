"""
统计：澳门心水推荐主胜或客胜，各大公司主胜或客胜上升时，澳门心水能否打出
"""
import os
import re
...
def analyze_odds_change(initial_odds, realtime_odds):
    """统计赔率变化"""
    home_up_10 = home_down = home_up = 0
    away_up_10 = away_down = away_up = 0
    ...
    return {
        "home_up_10": home_up_10, "home_up": home_up, "home_down": home_down,
        "away_up_10": away_up_10, "away_up": away_up, "away_down": away_down,
        "total": total
    }
...
def run_analysis():
    ...
    for match in all_matches:
        ...
        home_up_10_pct = stats['home_up_10'] / stats['total'] if stats['total'] > 0 else 0
        away_up_10_pct = stats['away_up_10'] / stats['total'] if stats['total'] > 0 else 0
        ...
        if macao_dir == "客胜":
            all_macao_away.append({
                ...
                'away_up_10_pct': away_up_10_pct,
                'actual': actual,
                'hit': actual == "客胜"
            })
    
    print("=" * 80)
    print("【澳门推荐主胜 + 赔率上升趋势分析】")
    print("=" * 80)
    ...
    for low, high, label in ranges:
        matches = [m for m in all_macao_home if low <= m['home_up_pct'] < high]
        if matches:
    ...
    print("\n按主胜大幅上升(>=10%)占比分段:")
    for low, high, label in ranges:
        matches = [m for m in all_macao_home if low <= m['home_up_10_pct'] < high]
        if matches:
            hits = sum(1 for m in matches if m['hit'])
            print(f"  {label}: {len(matches)}场, 打出{hits}场 ({hits/len(matches):.1%})")
    
    print("\n" + "=" * 80)
    print("【澳门推荐客胜 + 赔率上升趋势分析】")
    print("=" * 80)
    
    print("\n按客胜升(任意幅度)占比分段:")
    for low, high, label in ranges:
        matches = [m for m in all_macao_away if low <= m['away_up_pct'] < high]
        if matches:
            hits = sum(1 for m in matches if m['hit'])
            print(f"  {label}: {len(matches)}场, 打出{hits}场 ({hits/len(matches):.1%})")
    ...
    print("\n按客胜大幅上升(>=10%)占比分段:")
    for low, high, label in ranges:
        matches = [m for m in all_macao_away if low <= m['away_up_10_pct'] < high]
        if matches:
            hits = sum(1 for m in matches if m['hit'])
            print(f"  {label}: {len(matches)}场, 打出{hits}场 ({hits/len(matches):.1%})")
    ...
    # 打印符合条件的比赛详情
    print("\n" + "=" * 80)
    print("【详细比赛列表：澳门推荐主胜 + 主胜升>=30%公司】")
    print("=" * 80)
    
    for m in all_macao_home:
        if m['home_up_pct'] >= 0.3:
            print(f"{m['code']}: {m['home_team']} vs {m['away_team']}, 主胜{m['home']:.2f}, 升{m['home_up_pct']:.1%}, 实际{m['actual']}, {'O' if m['hit'] else 'X'}")
    
    print("\n【详细比赛列表：澳门推荐客胜 + 客胜升>=30%公司】")
    print("=" * 80)
    
    for m in all_macao_away:
        if m['away_up_pct'] >= 0.3:
            print(f"{m['code']}: {m['home_team']} vs {m['away_team']}, 客胜{m['away']:.2f}, 升{m['away_up_pct']:.1%}, 实际{m['actual']}, {'O' if m['hit'] else 'X'}")
