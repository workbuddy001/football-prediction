...
for key, s in scores.items():
    ...
    if ft.startswith('2026-04'):
        hhad = data.get('hhad', {})
        if hhad.get('让球', '') == '+1':
            apr_matches.append((key, int(hs), int(aw)))

print(f"找到 {len(apr_matches)} 场4月份+1让球有比分的比赛")
print("=" * 80)

results = []
skipped = 0
for mid, hs, aw in apr_matches:
    ...
    results.append({
        'match_id': mid, 'handicap': handicap,
        'prediction': pred, 'actual': ad,
        'actual_score': f'{hs}:{aw}',
        'pred_score': f'{pred_score[0]}:{pred_score[1]}' if pred_score else '-',
        'direction_correct': direction_ok, 'top3_hit': top3_hit,
        'invest': round(match_investment, 2), 'return': round(match_return, 2),
    })
...
hit = [r for r in results if r['top3_hit']]
miss = [r for r in results if not r['top3_hit']]
print(f"\n命中{len(hit)}场（投{sum(r['invest'] for r in hit):.0f}元，回{sum(r['return'] for r in hit):.0f}元）")
print(f"亏损{len(miss)}场（投{sum(r['invest'] for r in miss):.0f}元，全亏）")

outfile = 'apr_results_v2.json'
