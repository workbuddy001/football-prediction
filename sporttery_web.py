        ...
        async function runV36Analysis(matchId) {
            ...
            try {
                ...
                overlay.innerHTML = '<div class="v36-modal">'
                    ...
                    + vetoHtml
                    + '</div>'
                    + (data.h9_analysis && data.h9_analysis.prediction ? '<div class="v36-section" style="border:1px solid ' + (data.h9_analysis.is_reverse ? '#ff5722' : (data.h9_analysis.is_high_conf ? '#4caf50' : '#ff9800')) + ';border-radius:6px;background:' + (data.h9_analysis.is_reverse ? '#2e1a1a' : (data.h9_analysis.is_high_conf ? '#1a2e1a' : '#2e1a1a')) + ';">' + '<h4>🤖 H9 最高历史比分矛盾法' + (data.h9_analysis.is_reverse ? ' <span style="color:#ff5722;font-size:12px;">[⚠️反向推荐]</span>' : '') + (data.h9_analysis.is_high_conf ? '' : ' <span style="color:#ff9800;font-size:12px;">[⚠️非高置信度]</span>') + '</h4>' + '<div style="font-size:13px;line-height:1.8">' + '预测方向: <strong style="color:' + (data.h9_analysis.is_reverse ? '#ff5722' : (data.h9_analysis.is_high_conf ? '#4caf50' : '#ff9800')) + '">' + data.h9_analysis.prediction + '</strong><br>' + '置信度: <strong>' + (parseFloat(data.h9_analysis.confidence) || 0).toFixed(1) + '%</strong><br>' + '<div style="color:#b0b0b0;font-size:12px;line-height:1.6;word-break:break-all;margin-top:4px;">' + (data.h9_analysis.explanation || '') + '</div>' + '</div>' + '</div>' : '')
+ '<div class="v36-section">'
                    + '<h4>Step4 三维排除</h4>'
                    ...
                    + (a.final_goal_pick && a.final_goal_pick.single ? (function() {
                        ...
                        if (fgp.skip_reason && fgp.skip_reason.length > 0) {
                            ...
                            g += '<br><span style="color:#4caf50;font-size:18px;font-weight:bold;letter-spacing:4px">✅ 建议投注 ✅</span>';
                        }
                        g += '</div>';
                        return g;
                    })() : '')
                    ...
                    + (a.betting ? (function() {
                        ...
                        const goalColors = {'R0':'#e91e63','R1':'#4caf50','R3':'#ff9800','R4':'#2196f3','F':'#ff5722','S1':'#ff9800','S5':'#9c27b0','S4':'#ff5722','S3':'#e040fb','S2':'#ff9800','G4':'#ff9800','S6':'#2196f3','G5':'#e91e63','G6':'#9c27b0','G7':'#ff5722','H3':'#e040fb','H2':'#e91e63','H1':'#00bcd4'};
                        ...
                        const roiMap = {'R0':'+164%','R1':'+300%','R3':'+56%','R4':'探索中','F':'+275%','S1':'+80%','S5':'+910%','S4':'+1700%','S3':'+427%','S2':'+300%','G4':'+122%','S6':'+100%','G5':'+135%','G6':'+298%','G7':'+550%','H4':'+233%','H3':'+405%','H2':'+275%','H1':'+263%'};
