# -*- coding: utf-8 -*-
"""
V4(欧赔核心思维优化版) 预测 vs 3.15实际结果 对比
"""
...
print("=" * 95)
print("V4(欧赔核心思维优化版) 预测 vs 3.15 实际结果 对比")
print("=" * 95)
print(f"{'编号':<8} {'V4预测':<10} {'把握度':<6} {'实际结果':<10} {'比分':<8} {'正确?'}")
print("-" * 95)
...
for match_id in v4_pred.keys():
    v4, conf = v4_pred[match_id]
    actual = actual_315.get(match_id, '')
    score = scores.get(match_id, '')
    
    is_correct = v4 == actual
    ...
    mark = " *" if conf == 'B' else ""
    print(f"{match_id:<8} {v4:<10} {conf:<6} {actual:<10} {score:<8} {result}{mark}")

print("-" * 95)
print(f"总场次: {len(v4_pred)}")
print(f"V4正确: {correct}/{len(v4_pred)} = {correct/len(v4_pred)*100:.1f}%")
...
print("V3 vs V4 对比")
print("=" * 95)
print(f"V3(欧赔核心)准确率: 48.3%")
print(f"V4(优化版)准确率:   {correct/len(v4_pred)*100:.1f}%")
