...
def v6_predict(row, wl_data, macao_data):
    ...
    if macao_tip and wl:
        w_home, w_away = wl[0], wl[2]
        l_home, l_away = wl[3], wl[5]
        avg_home = (w_home + l_home) / 2
        avg_away = (w_away + l_away) / 2
        avg_draw = (wl[1] + wl[4]) / 2
        
        if macao_tip == "主胜":
            if avg_home < avg_away:
                return "主胜", distribution
            else:
                return "客胜" if avg_away < 2.5 else "防平", distribution
        elif macao_tip == "客胜":
            if avg_away < avg_home:
                return "客胜", distribution
            else:
                return "主胜" if avg_home < 2.5 else "防平", distribution
        elif macao_tip == "和局":
            if avg_draw < 3.3:
                return "防平", distribution
            else:
                return "主胜" if avg_home < avg_away else "客胜", distribution
...
for date, filepath in [('3.13', '3.13_V3预测.xlsx'), ('3.14', '3.14_比赛预测汇总.xlsx'), ('3.15', '3.15_V3预测.xlsx')]:
    df = pd.read_excel(filepath)
    for _, row in df.iterrows():
        code = row['编号']
        if code not in actual:
            continue
        pred, dist = v6_predict(row, WILLIAM_LADBROKES, MACAO_TIP)
        actual_result = actual[code]
        is_correct = pred == actual_result or ('防平' in pred and actual_result == '平')
        if dist not in dist_stats:
            dist_stats[dist] = {'correct': 0, 'total': 0}
...
print("V6算法 - 按分布类型准确率")
print("=" * 60)
for dist in ['顺分布', '逆分布', '缓冲分布', '中庸分布']:
    if dist in dist_stats:
        s = dist_stats[dist]
        rate = s['correct'] / s['total'] * 100
        print(f"{dist}: {s['correct']}/{s['total']} = {rate:.1f}%")
print("=" * 60)
