...
DATA_DIR = "d:/work/workbuddy/足球预测/分析模板/3.20"
MEMORY_FILE = "d:/work/workbuddy/足球预测/.workbuddy/memory/MEMORY.md"
...
def apply_rules(match):
    ...
    if confidence >= 66:
        # 判断澳门方向
        if '主' in macao or '贏' in macao and '主' not in macao and '客' not in macao:
            if h_change <= 0:  # 主降
                prediction = "主胜"
                rule = f"规律一:≥66%同向"
                return prediction, rule
...
def main():
    # 查找所有周五比赛
    friday_files = sorted(glob.glob(f"{DATA_DIR}/周五*_源数据.md"))
    ...
    # 输出
    print(f"\n{'='*80}")
    print(f"3.20 周五比赛分析（共{len(matches)}场）")
    print(f"{'='*80}\n")
