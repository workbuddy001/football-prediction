...
def _extract_recent_matches(data: dict, team_names: dict = None) -> dict:
    """
    ...
    for side in ['home', 'away']:
        ...
        target_team = team_names.get(side, '')
        matches = []
...
def predict_3goals(features: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if form is not None:
        ...
        _1goal_hit_rate_map = {}
        try:
            with open('分析模板/_1goal_hit_rate_map.json', 'r', encoding='utf-8') as f:
                _1goal_hit_rate_map = json.load(f)
        except:
            pass
        
        # 获取当前比赛的"1球变化%"
        _1goal_change_pct = features.get('1球变化%', 0)
        _1goal_bucket = round(_1goal_change_pct / 5) * 5
        _1goal_hit_rate = _1goal_hit_rate_map.get(str(_1goal_bucket), 0)
        
        # 近况均值>=3.5 → 排除1球 (6/79=7.6%)
        if combined_avg >= 3.5:
            # 例外条件：如果"1球变化命中率>20%"，则不完全排除
            if _1goal_hit_rate > 20:
                signals.append(('⚠️谨慎排除1球', '-3', f'近况均值{combined_avg}>=3.5，但1球变化命中率{_1goal_hit_rate}%>20%'))
                warnings.append(f'⚠️ 谨慎排除1球！近况均值{combined_avg}>=3.5，但1球变化命中率{_1goal_hit_rate}%>20%')
            else:
        ...
        if g1_val is not None and g1_val > 8.0:
            # 例外条件：如果"1球变化命中率>20%"，则不完全排除
            if _1goal_hit_rate > 20:
                signals.append(('⚠️谨慎排除1球', '-2', f'1球={g1_val}>8.0，但1球变化命中率{_1goal_hit_rate}%>20%'))
                warnings.append(f'⚠️ 谨慎排除1球！1球={g1_val}>8.0，但1球变化命中率{_1goal_hit_rate}%>20%')
            else:
