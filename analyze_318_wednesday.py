...
DATA_DIR = "d:/work/workbuddy/足球预测/分析模板/3.18"
...
def apply_rules(match):
    ...
    if confidence >= 66:
        # 判断澳门方向
        if '主' in macao or ('贏' in macao and '客' not in macao):
            if h_change <= 0:  # 主降
                prediction = "主胜"
                rule = f"规律一:≥66%同向"
                return prediction, rule
...
def main():
    # 查找所有周三比赛
    wednesday_files = sorted(glob.glob(f"{DATA_DIR}/周三*_源数据.md"))
    
    matches = []
    for f in wednesday_files:
    ...
    # 输出
    print(f"\n{'='*80}")
    print(f"3.18 周三比赛分析（共{len(matches)}场）")
    print(f"{'='*80}\n")
