...
for idx, row in df.iterrows():
    ...
    if panxing == '诱盘' and confidence > 50:
        ...
        # 实盘低置信 → 双选(原预测 + 除原预测外概率最高的)
        dual_pick = f"{pred_str}或{second_best}"
        dual_hit = (pred_str == actual_str) or (second_best == actual_str)
        reason = "实盘低置信→双选"
...
print("策略汇总")
print("=" * 95)
for strategy in result_df['原因'].unique():
    subset = result_df[result_df['原因'] == strategy]
    orig_c = (subset['原对错'] == 'O').sum()
    dual_c = (subset['双选中'] == 'O').sum()
    print(f"{strategy}: {dual_c}/{len(subset)} = {dual_c/len(subset)*100:.1f}%")
