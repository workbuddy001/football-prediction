...
for date, weekday, json_date in dates:
    ...
    for f in sorted(files):
        ...
        lookup_key = f"{date}_{date_num}" if date_num else f"{date}_{match_key}"
        ...
        try:
            ...
            print(f"{date} {num} {teams[:10]}: V7={v7_pred}({v7_prob:.0f}%) 8[{home_8},{draw_8},{away_8}] → {final_pred} | {actual} {score} {result}")
...
# 保存到文件
output_lines = []
output_lines.append("日期\t编号\t对阵\tV7预测\t置信度\t胜率差\t主胜8\t平局8\t客胜8\t增加最多\t最终预测\t实际\t比分\t结果")
for r in all_results:
    output_lines.append(f"{r['date']}\t{r['num']}\t{r['teams']}\t{r['v7_pred']}\t{r['confidence']}\t{r['diff']}\t{r['home_8']}\t{r['draw_8']}\t{r['away_8']}\t{r['increase']}\t{r['final_pred']}\t{r['actual']}\t{r['score']}\t{r['result']}")

with open('v7_8_analysis_result.txt', 'w', encoding='utf-8') as f:
    f.write('\n'.join(output_lines))

print("\n结果已保存到 v7_8_analysis_result.txt")
