...
def get_8_changes(date_str, match_key):
    """从JSON获取末尾8变化"""
    ...
    for key in data:
        if match_key in key or key.endswith(match_key):
            m = data[key]
            return {
                'home_8': m.get('home_8_change', 0),
                'draw_8': m.get('draw_8_change', 0),
                'away_8': m.get('away_8_change', 0),
            }
    return None
...
def analyze_with_8_weight(v7_result, changes):
    """用末尾8变化调整权重"""
    if not changes:
        return v7_result['v7_pred'], "-"
...
for date, weekday, json_date in dates:
    ...
    for f in sorted(files):
        ...
        try:
            ...
            # 获取8变化
            changes = get_8_changes(json_date, match_key)
            
            # 最终预测
            final_pred, increase_info = analyze_with_8_weight(v7, changes)
            ...
            home_8 = changes['home_8'] if changes else 0
            draw_8 = changes['draw_8'] if changes else 0
            away_8 = changes['away_8'] if changes else 0
            ...
            print(f"{date} {num} {teams}: V7={v7_pred}({v7_prob:.0f}%) → 最终={final_pred} | 实际={actual} {score} {result}")
...
for r in all_results:
    print(f"{r['date']}\t{r['num']}\t{r['teams'][:12]:12s}\t{r['v7_pred']}\t{r['confidence']}\t{r['diff']}\t{r['home_8']}\t{r['draw_8']}\t{r['away_8']}\t{r['increase']}\t{r['final_pred']}\t{r['actual']}\t{r['score']}\t{r['result']}")
