...
def analyze_match_v9(data):
    ...
    # 规则1: 极强队主场 (主胜<1.4) -> 必博主胜
    if avg_home < 1.4:
        first_choice = f"{data['home_team']}主胜"
        first_prob = f"{avg_home_prob:.0f}%"
        reason = "极强队主场"
    
    # 规则2: 极强队客场 (客胜<1.4) -> 必博客胜
    elif avg_away < 1.4:
        first_choice = f"{data['away_team']}客胜"
        first_prob = f"{avg_away_prob:.0f}%"
        reason = "极强队客场"
    
    # 规则3: 强队主场(1.4-1.55) + 澳门支持 -> 主胜
    elif avg_home < 1.55 and ("主" in macao_tip and "客" not in macao_tip):
        first_choice = f"{data['home_team']}主胜"
        first_prob = f"{avg_home_prob:.0f}%"
        reason = "强队主场+澳门支持"
    
    # 规则4: 强队客场(1.4-1.55) + 澳门支持 -> 客胜
    elif avg_away < 1.55 and "客" in macao_tip:
        first_choice = f"{data['away_team']}客胜"
        first_prob = f"{avg_away_prob:.0f}%"
        reason = "强队客场+澳门支持"
    ...
    # 规则7: 主队近况极佳(W>=4) + 主胜<2.5 -> 主胜
    elif home_wins >= 4 and avg_home < 2.5:
        first_choice = f"{data['home_team']}主胜"
        first_prob = f"{avg_home_prob:.0f}%"
        reason = "主队近况极佳"
    
    # 规则8: 客队近况极佳(W>=4) + 客胜<2.5 -> 客胜
    ...
    # 规则9: 主胜概率优势明显(>12%) -> 主胜
    elif avg_home_prob > avg_away_prob + 12 and avg_home_prob > avg_draw_prob + 10:
        first_choice = f"{data['home_team']}主胜"
        first_prob = f"{avg_home_prob:.0f}%"
        reason = "主胜概率优势明显"
    
    # 规则10: 客胜概率优势明显(>12%) -> 客胜
    elif avg_away_prob > avg_home_prob + 12 and avg_away_prob > avg_draw_prob + 10:
        first_choice = f"{data['away_team']}客胜"
        first_prob = f"{avg_away_prob:.0f}%"
        reason = "客胜概率优势明显"
    
    # 规则11: 平局概率突出(>35%) + 主客胜差距<12% -> 平局 (保守)
    elif avg_draw_prob > 35 and abs(avg_home_prob - avg_away_prob) < 12:
        first_choice = "平局"
