...
def extract_data(filepath):
    ...
    return {
        'num': num,
        'match': f"{home_match.group(1).strip()} vs {away_match.group(1).strip()}",
...
all_data.sort(key=lambda x: (x['date'], x['num']))
...
# 规律分析 - 基于总8变化
print("=" * 130)
print("3.12-3.16 比赛详细分析")
print("规律: 8变化+胜率差区间")
print("=" * 130)
print()
print("| 日期 | 编号 | 对阵 | 胜率差 | 区间 | 8变化 | 分析 | 预测 | 实际 | 结果 |")
print("|------|------|------|--------|------|-------|------|------|------|------|")
...
for d in all_data:
    ...
    print(f"| {d['date']} | {d['num']} | {d['match']} | {diff:+d}% | {zone} | {change_8:+d} | {analysis} | {pred} | {actual} | {result} |")

print()
print("=" * 130)
print(f"总命中率: {correct}/{total} = {correct/total*100:.1f}%")
