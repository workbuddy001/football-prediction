...
headers = ['编号', '赛事', '对阵', '澳门推荐', '盘型', '首选', '概率', '依据']
...
for r, data in enumerate(results, 2):
    ...
    ws.cell(row=r, column=6, value=data['first_choice'])
    ws.cell(row=r, column=7, value=data['first_prob'])
    ws.cell(row=r, column=8, value=data['reason'])
...
wb.save('3.15_比赛预测汇总_v4.xlsx')
print(f"Done: 3.15_比赛预测汇总_v4.xlsx")
