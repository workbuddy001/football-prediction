...
def analyze_match_v6(data):
    ...
    # 规则3: 主胜>5.0 + 客胜<1.8 + 客队近况好 -> 客胜
    elif avg_home > 5.0 and avg_away < 1.8 and away_wins >= 2:
        first_choice = f"{data['away_team']}客胜"
        first_prob = f"{avg_away_prob:.0f}%"
        reason = "弱主强客"
    
    # 规则4: 客胜>5.0 + 主胜<1.8 + 主队近况好 -> 主胜
    elif avg_away > 5.0 and avg_home < 1.8 and home_wins >= 2:
        first_choice = f"{data['home_team']}主胜"
        first_prob = f"{avg_home_prob:.0f}%"
        reason = "强主弱客"
    ...
    # 规则7: 主队近况很好(W>=4) + 主胜<2.5 -> 主胜
    elif home_wins >= 4 and avg_home < 2.5:
        first_choice = f"{data['home_team']}主胜"
        first_prob = f"{avg_home_prob:.0f}%"
        reason = "主队近况很好"
    
    # 规则8: 客队近况很好(W>=4) + 客胜<2.5 -> 客胜
    elif away_wins >= 4 and avg_away < 2.5:
        first_choice = f"{data['away_team']}客胜"
        first_prob = f"{avg_away_prob:.0f}%"
        reason = "客队近况很好"
    
    # 规则9: 平局概率最高且>32% -> 平局 (保守判断)
    elif avg_draw_prob > avg_home_prob and avg_draw_prob > avg_away_prob and avg_draw_prob > 32:
        first_choice = "平局"
        first_prob = f"{avg_draw_prob:.0f}%"
        reason = "平局概率最高"
    
    # 规则10: 主胜概率最高且优势明显(>8%) -> 主胜
    elif avg_home_prob > avg_away_prob + 8 and avg_home_prob > avg_draw_prob + 5:
        first_choice = f"{data['home_team']}主胜"
        first_prob = f"{avg_home_prob:.0f}%"
        reason = "主胜概率优势明显"
    
    # 规则11: 客胜概率最高且优势明显(>8%) -> 客胜
    elif avg_away_prob > avg_home_prob + 8 and avg_away_prob > avg_draw_prob + 5:
        first_choice = f"{data['away_team']}客胜"
        first_prob = f"{avg_away_prob:.0f}%"
        reason = "客胜概率优势明显"
