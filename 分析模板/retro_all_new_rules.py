...
# 3.17 比赛数据（从analyze_317_detailed.py）
matches_317 = [
    ("3.17", "周一001", "海尔蒙特 vs 坎布尔", "客胜", 58, -50, 0, 0, -5, "客胜"),
    ("3.17", "周一004", "布伦特 vs 狼队", "主胜", 64, +10, +1, +1, +1, "平局"),
    ("3.17", "周一006", "巴列卡诺 vs 莱万特", "主胜", 55, +29, +1, +1, +1, "平局"),
    ("3.17", "周二002", "中国女 vs Austral女", "客胜", 47, 0, 0, 0, 0, None),  # 待验证
    ("3.17", "周二004", "里斯本 vs 博德闪耀", "主胜", 62, +30, -2, 0, 0, None),
    ("3.17", "周二006", "阿森纳 vs 勒沃库森", "主胜", 73, +20, +2, 0, 0, None),
    ("3.17", "周二008", "曼城 vs 皇马", "主胜", 67, +10, 0, 0, 0, None),
    ("3.17", "周三001", "韩国女 vs 日本女", "客胜", 63, -20, -1, -1, -1, None),
]
...
print("=" * 110)
print()
print("| 日期 | 编号 | 对阵 | 胜率差 | 区间 | 主胜8 | 平局8 | 客胜8 | 按规律预测 | V7预测 | 实际 | 结果 |")
print("|------|------|------|--------|------|-------|-------|-------|------------|--------|------|------|")
...
for m in all_matches:
    ...
    # 8变化显示
    h8_str = f"{h8:+d}" if h8 != 0 else "0"
    d8_str = f"{d8:+d}" if d8 != 0 else "0"
    a8_str = f"{a8:+d}" if a8 != 0 else "0"
    
    actual_str = actual if actual else "-"
    
    print(f"| {date} | {num} | {match} | {diff:+d}% | {zone} | {h8_str} | {d8_str} | {a8_str} | {pred} | {v7} | {actual_str} | {result} |")
...
for zone in ["客队极好", "焦灼", "主队极好"]:
    ...
    if zone_total > 0:
        ...
        zone_stats[zone] = (zone_correct, zone_total, rate)
...
# 按日期统计
print("\n### 按日期统计")
for date in ["3.10", "3.11", "3.17"]:
    date_matches = [m for m in all_matches if m[0] == date]
    date_total = 0
    date_correct = 0
    ...
    if date_total > 0:
        rate = date_correct / date_total * 100
        print(f"{date}: {date_correct}/{date_total} = {rate:.1f}%")
