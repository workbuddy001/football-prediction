...
def load_predictions(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            if line.startswith('- '):
                # 提取比赛ID (如 周五001, 周六012 等)
                import re
                match = re.search(r'(周五|周六|周日)(\d+)', line)
                if match:
                    match_id = match.group(1) + match.group(2)  # 如 周五001
                    # 提取预测结果
                    if '**平局**' in line:
...
if __name__ == "__main__":
    # 读取三个文件的预测
    predictions_313 = load_predictions("3.13_V10预测.txt")
    predictions_314 = load_predictions("3.14_V10预测.txt")
    predictions_315 = load_predictions("3.15_V10预测.txt")
