...
for key, s in scores.items():
    ...
    if ft.startswith('2026-05'):
        hhad = data.get('hhad', {})
        if hhad.get('让球', '') == '+1':
            may_matches.append((key, int(hs), int(aw)))

print(f"找到 {len(may_matches)} 场5月份+1让球有比分的比赛")
print("=" * 80)

results = []
skipped = 0
for mid, hs, aw in may_matches:
