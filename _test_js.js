    ...
    if (simReviews.length > 0) {
        ...
        simReviews.forEach(function(rev) {
            ...
            if (odds.jc_real_odds && odds.jc_real_odds.length >= 3) {
                ...
                // 当前比赛的让球盘赔率（第四节竞彩让球赔率）
                var curHcJcOdds = [];
                var baseAna = res.analysis || {};
                var _jcOdds = baseAna.jc_odds;
                if (_jcOdds && Array.isArray(_jcOdds) && _jcOdds.filter(function(x){return x>0;}).length >= 2) {
                    curHcJcOdds = [_jcOdds[0]||0, _jcOdds[1]||0, _jcOdds[2]||0];
                }
