...
print(f"找到 {len(june_matches)} 场6月份有比分的比赛")
print("=" * 80)
...
miss_ret = sum(r['return'] for r in miss_results)

print(f"\n命中场次：{len(hit_results)}场（投注{hit_inv:.0f}元，回报{hit_ret:.0f}元，回报率{hit_ret/hit_inv*100:.0f}%）")
print(f"亏损场次：{len(miss_results)}场（投注{miss_inv:.0f}元，全亏）")
...
for r in results[:20]:
    ...
    print(f"{r['match_id']} 让球{r['handicap']} 实际{r['actual_score']} | "
          f"方向{ds} 前3{ts} | 预测{r['pred_score']} | 候选[{r['top3']}] | "
          f"投{r['invest']:.0f}元 回{r['return']:.0f}元 {pl:+.0f}元")

# 保存详细结果
outfile = 'june_results.json'
