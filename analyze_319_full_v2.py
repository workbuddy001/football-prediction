"""
3.19 比赛 - 完整预测列表（显示8变化、置信度详情、澳门推荐、赔率变化）
"""
...
# 赔率变化统计（从源数据提取）
odds_change_stats = {
    "周四001": {"home_down": 18, "total": 30, "draw_up": 5, "away_up": 22},
    "周四002": {"home_down": 12, "total": 30, "draw_up": 10, "away_up": 8},
    "周四003": {"home_down": 20, "total": 30, "draw_up": 3, "away_up": 7},
    "周四004": {"home_down": 8, "total": 30, "draw_up": 12, "away_up": 10},
    "周四005": {"home_up": 8, "total": 30, "draw_up": 8, "away_down": 14},
    "周四006": {"home_down": 22, "total": 30, "draw_up": 3, "away_up": 5},
    "周四007": {"home_down": 16, "total": 30, "draw_up": 6, "away_up": 8},
    "周四008": {"home_down": 18, "total": 30, "draw_up": 5, "away_up": 7},
    "周四009": {"home_down": 14, "total": 30, "draw_up": 8, "away_up": 8},
    "周四010": {"home_down": 6, "total": 30, "draw_up": 6, "away_down": 18},
    "周五001": {"home_up": 6, "total": 30, "draw_up": 10, "away_down": 14},
    "周五002": {"home_down": 16, "total": 30, "draw_up": 6, "away_up": 8},
    "周五003": {"home_up": 6, "total": 30, "draw_up": 10, "away_down": 14},
    "周五004": {"home_up": 4, "total": 30, "draw_up": 6, "away_down": 20},
    "周五005": {"home_down": 16, "total": 30, "draw_up": 10, "away_up": 4},
    "周五006": {"home_up": 8, "total": 30, "draw_up": 8, "away_down": 14},
    "周五007": {"home_down": 14, "total": 30, "draw_up": 10, "away_up": 6},
    "周五008": {"home_down": 10, "total": 30, "draw_up": 10, "away_up": 10},
    "周五009": {"home_down": 20, "total": 30, "draw_up": 3, "away_up": 7},
    "周五010": {"home_down": 20, "total": 30, "draw_up": 3, "away_up": 7},
    "周五011": {"home_up": 10, "total": 30, "draw_up": 8, "away_down": 12},
    "周五012": {"home_down": 22, "total": 30, "draw_up": 3, "away_up": 5},
    "周五013": {"home_up": 6, "total": 30, "draw_up": 10, "away_down": 14},
    "周五014": {"home_down": 8, "total": 30, "draw_up": 10, "away_up": 12},
    "周五015": {"home_down": 16, "total": 30, "draw_up": 8, "away_up": 6},
    "周五016": {"home_up": 8, "total": 30, "draw_up": 12, "away_down": 10},
}
...
def get_odds_change_str(match_id):
    """获取赔率变化统计字符串"""
    stats = odds_change_stats.get(match_id, {"home_down": 0, "total": 30, "draw_up": 0, "away_up": 0})
    total = stats.get("total", 30)
    
    # 主胜变化
    if "home_down" in stats:
        home_pct = stats["home_down"] / total * 100
        home_str = f"主↓{home_pct:.0f}%"
    elif "home_up" in stats:
        home_pct = stats["home_up"] / total * 100
        home_str = f"主↑{home_pct:.0f}%"
    else:
        home_str = "主-"
    
    # 平局变化
    draw_pct = stats.get("draw_up", 0) / total * 100
    draw_str = f"平↑{draw_pct:.0f}%"
    
    # 客胜变化
    if "away_up" in stats:
        away_pct = stats["away_up"] / total * 100
        away_str = f"客↑{away_pct:.0f}%"
    elif "away_down" in stats:
        away_pct = stats["away_down"] / total * 100
        away_str = f"客↓{away_pct:.0f}%"
    else:
        away_str = "客-"
    
    return f"{home_str} {draw_str} {away_str}"
...
for mid, data in matches_data.items():
    ...
    # 赔率变化
    odds_change = get_odds_change_str(mid)
...
print("=" * 160)
print("3.19 比赛 - 完整预测列表（显示8变化、置信度详情、澳门推荐、赔率变化）")
print("=" * 160)

print(f"\n| 编号 | 对阵 | 置信度详情 | 置信度 | 胜率差 | 8变化 | 赔率变化 | 澳门 | 偏离 | 预测 |")
print(f"|------|------|------------|--------|--------|-------|----------|------|------|------|")
...
for r in results:
    ...
    print(f"| {r['id']} | {r['match']} | {conf_detail} | {r['confidence']:.1f}% | {r['rate_diff']:+.1f}% | {eight_str} | {r['odds_change']} | {r['macao']} | {r['deviation_type']} | {r['prediction']}{zhongyong_mark} |")
...
print("【偏离过高】最可信 (2场)")
print("=" * 160)
print(f"\n| 编号 | 对阵 | 置信度详情 | 置信度 | 胜率差 | 8变化 | 赔率变化 | 澳门 | 预测 |")
print(f"|------|------|------------|--------|--------|-------|----------|------|------|")
for r in high_dev:
    conf_detail = f"主{r['home_rate']:.0f}% 平{r['draw_rate']:.0f}% 客{r['away_rate']:.0f}%"
    ec = r['eight_change']
...
# 澳门推荐 vs 预测 一致性
match_count = sum(1 for r in results if r['macao'] != "-" and r['macao'] in ["主胜", "客胜", "和局"])
macao_match = sum(1 for r in results if (r['macao'] == "主胜" and r['prediction'] == "主胜") or (r['macao'] == "客胜" and r['prediction'] == "客胜") or (r['macao'] == "和局" and r['prediction'] == "平局"))
