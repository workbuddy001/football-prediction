        ...
        for line in lines:
            ...
            if in_table:
                if line.startswith('---') or line.startswith('>'):
                    break
                # 表格格式: | 公司 | 初盘胜 | 即时胜 | 变动 | 初盘平 | 即时平 | 变动 | 初盘负 | 即时负 | 变动 |
                # 分割后索引: 0=空, 1=公司, 2=初盘胜, 3=即时胜, 4=变动, 5=初盘平, 6=即时平, 7=变动, 8=初盘负, 9=即时负, 10=变动
                if '|' in line and ('↓' in line or '↑' in line or '—' in line):
                    parts = [p.strip() for p in line.split('|')]
                    # 第4个位置是主胜变动(索引4)，第7个是平局变动(索引7)，第10个是客胜变动(索引10)
...
print("=" * 200)
print("3.19 比赛 - 完整预测列表")
print("=" * 200)
print("赔率变化说明: 主降/稳/升 = 主胜降赔/不变/升赔的公司百分比; 平降/稳/升 = 平局降/不变/升; 客降/稳/升 = 客胜降/不变/升")

print(f"\n| 编号 | 对阵 | 置信度 | 胜率差 | 8变化 | 赔率变化(主降/稳/升 平降/稳/升 客降/稳/升) | 澳门 | 预测 |")
print(f"|------|------|--------|--------|-------|------------------------------------------|------|------|")
...
for r in results:
    ...
    print(f"| {r['id']} | {r['match']} | {r['confidence']:.1f}% | {r['rate_diff']:+.1f}% | {eight_str} | {r['odds_change']} | {r['macao']} | {r['prediction']}{zhongyong_mark} |")
...
print("【偏离过高】最可信 (2场)")
print("=" * 200)
print(f"\n| 编号 | 对阵 | 置信度 | 胜率差 | 8变化 | 赔率变化 | 澳门 | 预测 |")
print(f"|------|------|--------|--------|-------|----------|------|------|")
for r in high_dev:
    ec = r['eight_change']
    eight_str = f"[{ec[0]:+d},{ec[1]:+d},{ec[2]:+d}]"
    print(f"| {r['id']} | {r['match']} | {r['confidence']:.1f}% | {r['rate_diff']:+.1f}% | {eight_str} | {r['odds_change']} | {r['macao']} | {r['prediction']} |")
...
print(f"\n| 类型 | 场数 | 主胜 | 客胜 |")
print(f"|------|------|------|------|")
print(f"| 偏离过高 | {len(high_dev)} | {sum(1 for r in high_dev if r['prediction']=='主胜')} | {sum(1 for r in high_dev if r['prediction']=='客胜')} |")
print(f"| 正常偏离 | {len(normal_dev)} | {sum(1 for r in normal_dev if r['prediction']=='主胜')} | {sum(1 for r in normal_dev if r['prediction']=='客胜')} |")
print(f"| 偏离过低 | {len(low_dev)} | {sum(1 for r in low_dev if r['prediction']=='主胜')} | {sum(1 for r in low_dev if r['prediction']=='客胜')} |")
print(f"| 总计 | {len(results)} | {sum(1 for r in results if r['prediction']=='主胜')} | {sum(1 for r in results if r['prediction']=='客胜')} |")
